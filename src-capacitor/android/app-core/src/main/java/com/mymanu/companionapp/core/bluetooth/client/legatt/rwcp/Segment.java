...
package com.mymanu.companionapp.core.bluetooth.client.legatt.rwcp;
...
class Segment {
     ...
     */
    private final RWCPData mPayload;
    /**
     ...
     */
    Segment(int operationCode, int sequenceNumber, RWCPData payload) {
    ...
    Segment(int operationCode, int sequenceNumber) {
        this(operationCode, sequenceNumber, new RWCPData(new byte[0], null));
    }
     ...
     * <ul>
     *     <li>operation code: {@link RWCP.OpCode#NONE RWCP.OpCode.NONE},</li>
    ...
    Segment(byte[] bytes) {
        ...
        if (bytes == null || bytes.length < RWCP.Segment.REQUIRED_INFORMATION_LENGTH) {
            Log.w(TAG, "Building of RWCP Segment failed: the byte array does not contain the minimum " +
                    "required information.\nbytes: " + BytesUtils.getHexadecimalStringFromBytes(bytes));
            mOperationCode = RWCP.OpCode.NONE;
            mSequenceNumber = -1;
            mHeader = -1;
            mPayload = new RWCPData(bytes==null ? new byte[0] : bytes, null);
        }
        else {
            ...
            mPayload = new RWCPData(new byte[bytes.length - RWCP.Segment.HEADER_LENGTH], null);
        }
    }
    ...
    byte[] getBytes() {
        if (mBytes == null) {
            int payloadLength = (mPayload == null) ? 0 : mPayload.getBytes().length;
            mBytes = new byte[RWCP.Segment.REQUIRED_INFORMATION_LENGTH + payloadLength];
            mBytes[RWCP.Segment.HEADER_OFFSET] = mHeader;
            // data if exists
            if (payloadLength > 0) {
                System.arraycopy(mPayload.getBytes(), 0, mBytes, RWCP.Segment.PAYLOAD_OFFSET, mPayload.getBytes().length);
            }
        }
     ...
     */
    private String toString(boolean showPayload) {
        StringBuilder builder = new StringBuilder();
        builder.append("[code=").append(mOperationCode).append(", sequence=").append(mSequenceNumber);
        if (showPayload) {
            builder.append(", payload=").append(BytesUtils.getHexadecimalStringFromBytes(mPayload.getBytes()));
        }
        builder.append("]");
