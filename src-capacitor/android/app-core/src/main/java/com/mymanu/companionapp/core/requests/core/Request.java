...
package com.mymanu.companionapp.core.requests.core;
 ...
 * <p>See {@link RequestManager} for executing
...
public abstract class Request<Result, Progress, Error> {
    ...
    protected void onComplete(Result result) {
        ...
        if (mListener != null) {
            GaiaClientService.getTaskManager().runOnMain(() -> mListener.onComplete(result));
        }
        onEnd();
    }
    ...
    protected void onError(Error reason) {
        ...
        if (mListener != null) {
            GaiaClientService.getTaskManager().runOnMain(() -> mListener.onError(reason));
        }
        onEnd();
    }
    ...
    protected void onProgress(Progress progress) {
        ...
        if (mListener != null) {
            GaiaClientService.getTaskManager().runOnMain(() -> mListener.onProgress(progress));
        }
    }
