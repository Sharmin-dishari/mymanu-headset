...
package com.mymanu.companionapp.ui
...
import com.mymanu.companionapp.databinding.ActivityUpdateBinding
import com.mymanu.companionapp.ui.common.Observers
import com.mymanu.companionapp.ui.common.SnackMessage
...
@AndroidEntryPoint
class UpdateActivity : PermissionsActivity() {
...
//        setSupportActionBar(findViewById(R.id.top_bar))
      // Init toolbar
//      setSupportActionBar(findViewById(R.id.top_bar))
      // Show back icon in the action bar
...
//    }
  ...
  override fun onBackPressed() {
    ...
    // Check if the current destination is one of the top-level destinations
    if (navController.currentDestination != null && NAVIGATION_WITHOUT_BACK_ARROW.contains(navController.currentDestination!!.id)) {
      // Close the activity if it's a top-level destination
      finish()
    } else {
      // Handle the back press as a navigation action
      super.onBackPressed()
    }
  }
    ...
    private inner class DataObservers : Observers() {
        ...
        private val bluetoothPermissionObserver =
            Observer { state: PermissionState -> this@UpdateActivity.onPermissionStateChanged(state) }

        override fun registerObservers() {
            viewModel.observeNavigationVisibility(this@UpdateActivity, navigationObserver)
            viewModel.observeIconBarVisibility(this@UpdateActivity, iconBarVisibilityObserver)
            viewModel.observeBluetoothRequestState(this@UpdateActivity, bluetoothRequestObserver)
            viewModel.observeSnackMessage(this@UpdateActivity, snackMessageObserver)
            permissionViewModel.getState(Permissions.Categories.BLUETOOTH)
                .observe(this@UpdateActivity, bluetoothPermissionObserver)
        }
