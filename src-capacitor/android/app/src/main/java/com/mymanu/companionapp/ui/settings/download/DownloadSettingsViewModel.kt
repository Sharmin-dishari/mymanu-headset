...
class DownloadSettingsViewModel @Inject constructor(
    ...
    private fun fetchDeviceInformation() {
        deviceInfoRepository.fetchDeviceInfo(application, DeviceInfo.APPLICATION_VERSION)
        deviceInfoRepository.fetchDeviceInfo(application, DeviceInfo.SYSTEM_INFORMATION)
    }

    private fun onVersions(versions: Versions?) {
        val label = versions?.application ?: application.getString(R.string.info_unknown)
    ...
    private fun updateApplicationBuildId(id: ApplicationBuildId) {
        val label = id.value.text ?: application.getString(R.string.info_unknown)
