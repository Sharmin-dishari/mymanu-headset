...
import com.mymanu.companionapp.repository.service.ServiceAvailabilityImpl.isAvailable
...
class MainSettingsViewModel @Inject constructor(
    ...
    fun disconnect() {
        connectionRepository.disconnect(application)
    }

    fun setAssistant(assistant: VoiceAssistant) {
        voiceUIRepository.setAssistant(application, assistant)
    }
    ...
    private fun updateSelectedAssistant(assistant: VoiceAssistant?) {
        val summary = assistant?.getLabel(application) ?: ""
    ...
    private fun getEntries(assistants: List<VoiceAssistant>): Pair<Array<String>, Array<String>> {
        ...
        val entries = assistants.map { it.getLabel(application) }.toTypedArray()
