...
import com.mymanu.companionapp.databinding.DialogStatisticsSettingsBinding
import com.mymanu.companionapp.databinding.FragmentStatisticsListBinding
import com.mymanu.companionapp.ui.UpdateActivity
import com.mymanu.companionapp.ui.common.MessageData
import com.mymanu.companionapp.utils.FileUtils
...
class StatisticsStatisticsFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        viewModel = ViewModelProvider(this)[StatisticsStatisticsViewModel::class.java]
    ...
    override fun onCreateView(
        ...
        adapter = StatisticsStatisticsAdapter(
            ...
            viewModel?.isSupported(QTILFeature.XPAN) == true,
            { viewModel?.toggleXpanTransport() },
            { viewModel?.setXpanTransport(it) },
            { viewModel?.setAPConnectionState(it) }
        )
    ...
    private fun updateTitle(title: String) {
        if (activity is UpdateActivity && title.isNotEmpty()) {
            (activity as UpdateActivity).updateTitle(title)
        }
    }
